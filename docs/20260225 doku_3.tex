...
\textbf{Deterministic Compilation of Large Language Models (LLMs):}
While modern LLMs are predominantly based on Transformer architectures trained by gradient descent on large corpora \cite{Vaswani2017,Brown2020}, this work explores an alternative viewpoint: for many domain-specific tasks, a large part of the usable ``language model'' can be realized as an explicit, compiled program derived from the structure of the stored data.

Eigentokens treats deduplication artifacts (chunks) as a token alphabet and learns reusable productions (a grammar cookbook). Instead of learning an opaque parameter tensor, the system compiles these productions and their co-occurrence statistics into an executable model image (e.g., compressed transition tables or weighted automata) that can be queried efficiently. Where neural backends are desirable, the same artifacts can be exported as training corpora or initialization to standard Transformer toolchains, but that is not required for the baseline compiled model.

This compilation-oriented view aims to provide:
\begin{itemize}
  \item \textbf{Traceability:} each suggestion can be linked to the productions and stored fragments that supported it.
  \item \textbf{Reproducibility:} given a snapshot of the database state, the compiled model is deterministic and can be rebuilt bit-for-bit.
  \item \textbf{Incremental updates:} grammar and statistics can be updated asynchronously without retraining a full neural model from scratch.
\end{itemize}
...
\item \textit{Deterministic model compilation} of an executable language model artifact directly from stored patterns (with optional export to neural backends),
...
\subsection{Formal Grammars and the Chomsky Hierarchy}
...
\subsection{Asynchronous Pipeline Dynamics}
...
\subsection{Fingerprints and Similarity Signals}
...
\item \textbf{Unified storage and compilation.} Existing databases optimize for either storage efficiency or query performance, not for compiling AI models from the stored patterns. Eigentokens require the storage system to simultaneously maintain a queryable B$^+$-forest index \emph{and} a ``grammar cookbook'' of production rules that can be compiled into a binary model image (and optionally mapped into neural backends). This dual purpose demands custom data structures and a processing pipeline that no off-the-shelf database provides.
...
\begin{itemize}
\item Production frequencies and co-occurrence statistics define an explicit transition model that can be compiled into a binary image (gradient-based training is optional when targeting neural backends).
...
\medskip
\noindent\textbf{Note on implementation inventory.} Quantitative implementation details (e.g., module counts, LOC estimates, and completion ratios) are moved to Appendix~\ref{app:implementation_inventory} to keep the main chapters focused on the scientific questions: \emph{what} is compiled and \emph{why} the architecture makes compilation plausible.
\medskip


\chapter{What We Compile: From ComdareDB State to an Executable Language Model}
\label{ch:compiled_model}

This chapter intentionally focuses on the scientific \emph{what} and \emph{why}. The quantitative question of \emph{how much} of the system is already implemented is documented separately in Appendix~\ref{app:implementation_inventory}.

\section{Definition of the compiled artifact: CELM as a binary program}
\label{sec:celm_artifact}

We use the term \textbf{Comdare Eigentoken Language Model (CELM)} to denote the \emph{compiled output} of ComdareDB.

Earlier design notes referred to the generic concept as the \textbf{Eigentoken Language Model (ELM)}; in this report, \textbf{CELM} names the Comdare-specific instantiation (ELM $\rightarrow$ CELM).
In contrast to mainstream Large Language Models (LLMs) that are typically instantiated as large neural networks trained by gradient descent \cite{Vaswani2017,Brown2020}, CELM is defined as an \emph{executable binary artifact} produced from the database state.

\paragraph{Inputs.} A CELM build consumes a consistent snapshot of:
(i) the content-addressable chunk store and B$^+$-forest index,
(ii) the grammar cookbook (learned productions over byte- and token-level structure),
and (iii) relational occurrence statistics over co-appearing chunks (forward and backward context).

\paragraph{Outputs.} The compilation step produces a deployable model package, conceptually similar to a compiler toolchain output:
\begin{itemize}
  \item a \emph{binary model image} (tables and/or compressed automata) encoding grammar productions and transition statistics,
  \item an \emph{execution runtime} (library or service) that answers queries against the compiled image,
  \item optional \emph{trace links} back to the originating stored chunks, enabling auditability and provenance.
\end{itemize}

\paragraph{Execution contract.} Given an input string (or token prefix), CELM returns:
(1) ranked candidate continuations,
(2) optionally the most similar historical contexts and their predecessors/successors,
and (3) a trace of which productions and stored fragments supported each suggestion.
This definition makes the model \emph{explicit} and \emph{inspectable} by design.

\section{Semantics: what CELM returns for an input string}
\label{sec:celm_semantics}

A practical compiled model must define how an input string is mapped onto internal units and how outputs are scored.

\paragraph{Tokenization as grammar application.}
Instead of applying a fixed tokenizer, CELM tokenization is a \emph{parsing step}:
the input is segmented into Eigentokens by applying the learned productions (e.g., a dictionary of reusable byte patterns, plus fallbacks for out-of-vocabulary regions).
This is aligned with classical grammar induction and grammar-based compression techniques such as Sequitur \cite{NevillManning1997} or Re-Pair \cite{LarssonMoffat2000}.

\paragraph{Scoring via explicit statistics.}
Once the input is represented as a sequence of Eigentokens, continuation candidates can be scored using explicit co-occurrence statistics, analogously to classical statistical language modeling (e.g., $n$-gram counts and backoff) rather than opaque neural weights.
Where needed, smoothing and backoff can be implemented with standard techniques \cite{Kneser1995,ChenGoodman1996}.

\paragraph{Output as program results.}
The core output is not merely a text string but a \emph{program result}:
ranked continuations plus the relational evidence graph that explains why these continuations are likely under the stored corpus distribution.



\subsection{Retrieval Output Contract}
...
\section{Grammatical chunking strategies and asynchronous grammar induction}
\label{sec:chunking_strategies}

ComdareDB relies on two complementary ideas:
\textbf{(i)} content-defined chunking for stable boundaries under edits, and
\textbf{(ii)} grammar induction to elevate recurring patterns into reusable productions.

\paragraph{Content-defined chunking (CDC).}
CDC (e.g., based on Rabin fingerprints) yields chunk boundaries that are robust under inserts and shifts, which is crucial for deduplication and for stable downstream statistics \cite{Rabin1981,Muthitacharoen2001}.
This property provides a stronger substrate for learning than fixed-size chunking when the data evolves.

\paragraph{Grammar induction on top of deduplication.}
Deduplication creates a natural ``alphabet'' of recurring fragments.
Grammar induction can then learn higher-level productions that compress and regularize those fragments into hierarchical structure \cite{NevillManning1997,LarssonMoffat2000}.
In ComdareDB, this induction is designed to be asynchronous: write paths remain fast, while grammar learning and statistics aggregation run in background phases.

\paragraph{From grammar to model image.}
The key architectural claim is that these learned productions and statistics are sufficient to compile a \emph{domain language model image}:
a compact representation that supports fast prefix queries and probabilistic continuation without requiring full neural training for the \emph{baseline} system.

\section{Module ecosystem: what ComdareDB links in}
\label{sec:module_ecosystem}

ComdareDB is not a monolithic program; it is assembled from a set of reusable modules (``Baselines) that provide the generic infrastructure needed for a storage-backed compiler: memory and platform abstraction, hashing/serialization primitives, concurrency utilities, protocol stacks, and storage backends.

At a high level, these linked modules can be grouped into:
\begin{itemize}
  \item \textbf{Foundation:} memory management, SIMD utilities, threading/concurrency, configuration, and debugging infrastructure.
  \item \textbf{Data primitives:} big integers, hashing, serialization and schema utilities, and container building blocks.
  \item \textbf{Protocols and I/O:} network and file-protocol adapters used by the storage interfaces.
  \item \textbf{Storage backends:} local/remote object stores and archive formats, including integrity checks and optional encryption.
  \item \textbf{Build orchestration:} CMake templating and XML-driven interpretation used to stage and validate build phases.
\end{itemize}

The full module inventory and the detailed implementation mapping are documented in Appendix~\ref{app:module_inventory}. In the main chapters, we treat these modules as enabling infrastructure for the scientific question of compiling language models from database structure.

\section{How CELM differs from contemporary LLM systems and techniques}
\label{sec:celm_vs_llm}

To position CELM, it is useful to separate three dimensions: \emph{representation}, \emph{learning/update mechanism}, and \emph{execution runtime}.

\begin{table}[ht]
\centering
\caption{Qualitative comparison between CELM and representative contemporary LLM techniques.}
\label{tab:celm_comparison}
\begin{tabularx}{\textwidth}{l X X}
\toprule
\textbf{Approach} & \textbf{Core mechanism} & \textbf{Key difference to CELM} \\
\midrule
Transformer LLMs \cite{Vaswani2017,Brown2020,Touvron2023LLaMA} & Gradient-trained neural sequence model (dense or MoE) & Knowledge is implicit in weights; CELM keeps grammar and statistics explicit and traceable. \\
MoE layers \cite{Shazeer2017MoE} & Sparse gating over expert subnetworks & Optimization complexity; CELM focuses on compilation from explicit corpus structure rather than expert routing. \\
Retrieval-Augmented Generation (RAG) \cite{Lewis2020RAG} & Neural generator + external retriever & RAG adds retrieval at inference; CELM integrates retrieval-like context via the database's relational chunk graph as a first-class model representation. \\
Parameter-efficient tuning (LoRA) \cite{Hu2021LoRA} & Low-rank adapters on pretrained weights & CELM can be built from stored data directly; adapters assume a pretrained neural core. \\
Deep learning compilers \cite{Chen2018TVM} & Optimize execution of a fixed ML graph & CELM is a \emph{data-to-model} compiler: it compiles a model image from the database state, not just the runtime of a given network. \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Interpretability and provenance.}
Because CELM can retain direct links from compiled transitions back to stored fragments, it enables an explanation style that conventional LLMs cannot provide without additional tooling.

\paragraph{Determinism as a design goal.}
By compiling explicit productions into a binary image, CELM enables deterministic replay and reproducible inference runs (given a snapshot), which is valuable for scientific evaluation and for system debugging.

\section{What existing research contributes to this direction}
\label{sec:celm_related_enablers}

CELM builds on several research lines:
\begin{itemize}
  \item \textbf{Deduplication and content-addressable storage:} stable chunking and efficient indexing in storage systems \cite{Muthitacharoen2001,Quinlan2002}.
  \item \textbf{Grammar-based compression and induction:} learning reusable productions from sequences \cite{NevillManning1997,LarssonMoffat2000}.
  \item \textbf{Classical explicit language models:} $n$-gram modeling and smoothing methods remain relevant when the goal is explicit, inspectable probability tables \cite{Kneser1995,ChenGoodman1996}.
  \item \textbf{Automata-based formalisms:} weighted finite-state representations provide efficient compiled runtimes for language-like distributions \cite{Mohri2002WFST}.
\end{itemize}

The novelty in ComdareDB is the \emph{unification} of these components in a single system:
storage and deduplication are not just an optimization for saving space, but a mechanism for learning grammar and compiling an executable model artifact.
...
\item[Model Compile:] Storage patterns and learned productions can be compiled into a deployable binary model image for efficient inference.
...
\chapter{Implementation Inventory and Progress}\label{app:implementation_inventory}
...
\chapter{Module Inventory and Implementation Detail}\label{app:module_inventory}

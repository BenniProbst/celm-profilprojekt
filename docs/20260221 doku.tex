% NOTE: KOMA's `open` option is irrelevant for `oneside` and triggers an
% "Unused global option" warning with the current zihpub wrapper.
...
\sisetup{
  ...
  % siunitx v3.4+: avoid deprecated detect-* options
  mode = match,
...
% Recommended with KOMA+hyperref to improve bookmark handling.
\usepackage{bookmark}

% Hyperref/bookmark safety for this document:
% - prevent duplicate page anchors
% - avoid warnings from unsupported tokens in PDF strings
% - reduce overfull boxes from unavoidable long technical identifiers
\AtBeginDocument{%
...
% Avoid math/superscripts in PDF bookmarks (hyperref): use a plain-string fallback.
\section{SHA512-Indexed B\texorpdfstring{$^+$}{+}-Forest for Token Storage}
...
% -----------------------------------------------------------------
% Comdare implementation chapter (reference DB + gap analysis)
% -----------------------------------------------------------------
